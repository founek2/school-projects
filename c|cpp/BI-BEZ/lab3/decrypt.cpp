...
int main(int argc, char *argv[]) {
    ...
    /* Decryption */
    printf("Running decryption with cipher=%s and key=%s\n", cipherName.c_str(), key);
    res = EVP_DecryptInit_ex(ctx, cipher, NULL, key, iv);  // nastaveni kontextu pro desifrovani
    if(res != 1) exit(6);


    const char *in_file = (FILE_NAME + "_" + toLower(argv[1]) + ".bmp").c_str();
    const char *out_file = (FILE_NAME + "_" + toLower(argv[1]) + "_dec.bmp").c_str();
    ...
    if (fh.is_open() && f_out.is_open()){
        ...
        fh.read(bytes, 4096);
        while (fh.gcount() > 0){
            res = EVP_DecryptUpdate(ctx, (unsigned char *) st_out, &len,  (const unsigned char *) bytes, fh.gcount());  // desifrovani st
            if(res != 1) exit(7);
    ...
    res = EVP_DecryptFinal_ex(ctx, (unsigned char *) st_out, &len);  // dokonceni (ziskani zbytku z kontextu)
    if(res != 1) exit(8);
    f_out.seekp(pos);
    f_out.write(st_out, len);
    ...
    printf("Porovnal jsem výsledek s původním pomocí cmp -> byly stejné");
