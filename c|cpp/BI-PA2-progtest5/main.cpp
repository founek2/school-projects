...
public:
    ...
    int CompareByTime(const CMail &x) const {
        return m_timeStamp.Compare(x.TimeStamp());
    }
...
//----------------------------------------------------------------------------------------
    ...
    int cmpF(const CMail &mail, const CTimeStamp &time) {
        return mail.CompareByTime(time) < 0;
    }

    int cmpFUpper(const CTimeStamp &time, const CMail &mail) {
        return mail.CompareByTime(time) > 0;
    }
            ...
            try {
                if (split(line, ' ', fields, 6) == 7) {
                    ...
                    if (split(fields[6], '=', actions, 1) == 2) {
                        ...
                        if (!is_number(fields[1]) || !is_number(fields[2])) throw invalid_argument("Invalid number.");
                        ...
                        if (actions[0] == "from") {
                            ...
                            if (split(fields[3], ':', time, 2) == 3) {
                                if (!is_number(time[0]) || !is_number(time[1])) throw invalid_argument("Invalid number.");
                                auto it = mp.find(*id);

                                if (it == mp.end()) throw invalid_argument("missing from.");

                                it->second.m_to.emplace_back(make_pair(actions[1],
...
/*        for (auto const &x: mp) {
        ...
        sort(emails.begin(), emails.end(), [](const CMail &mail1, const CMail &mail2) {
            return mail1.CompareByTime(mail2) < 0;
        });
    ...
    list<CMail> CMailLog::ListMail(const CTimeStamp &from, const CTimeStamp &to) const {
        ...
        const auto last = upper_bound(emails.begin(), emails.end(), to, cmpFUpper);

        return list<CMail>(first, last);
    }
