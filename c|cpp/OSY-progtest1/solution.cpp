...
void CSentinelHacker::workerMain() {
    ...
    while(this->popFragment(data)){
...

        {
            ...
            this->problems[id].push_back(data);     // may be slow
            array = this->problems[id];
            len = array.size();
        }
        CBigInt out;
        //cout << "SeqSolve2" << std::endl;
        if (this->SeqSolve2(array, len, out)){
            {
                // cout << "found solution" << std::endl;
                std::lock_guard<std::mutex> lockGuard2(mut_results);
                this->qResults.push(std::make_pair(id, out));
            }
            cv_result.notify_one();
            {
                std::lock_guard<std::mutex> lockGuard(mut_problems);
                this->problems.erase(id);
            }
        }
    }
}
...
bool CSentinelHacker::popFragment(uint64_t & data) {
    ...
    while  (true) {
        cv_fragment.wait(lk, [this]{return !this->qFragments.empty() || shouldStop;});
        if (this->qFragments.empty() && shouldStop) return false;
        else if (!this->qFragments.empty()) break;
    }
...
bool CSentinelHacker::popResult(std::pair<uint32_t, CBigInt> & elem) {
    ...
    while  (true) {
        cv_result.wait(lk, [this]{return !this->qResults.empty() || workersDeath;});
        if (this->qResults.empty() && workersDeath) return false;
        else if (!this->qResults.empty()) break;
    }
